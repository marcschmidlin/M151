	...
	if (empty($error)) {
		...
		if ($row = $result->fetch_assoc()) {
			...
			if (password_verify($password, $row['Passwort'])) {
				...
				// Query erstellen
				$query = "UPDATE benutzer SET Passwort = ? WHERE email = ?";
				...
				<div class="wrap d-md-flex">
					...
					<div class="login-wrap p-4 p-md-5">
						...
						<form method="POST">
							...
							<!-- password -->
							<div class="form-group">
								<label for="newPasswort">Neues Password *</label>
								<input type="password" name="newPasswort" class="form-control" id="newPasswort" placeholder="Gross- und Kleinbuchstaben, Zahlen, Sonderzeichen, min. 8 Zeichen, keine Umlaute" pattern="(?=^.{8,}$)((?=.*\d+)(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" title="mindestens einen Gross-, einen Kleinbuchstaben, eine Zahl und ein Sonderzeichen, mindestens 8 Zeichen lang,keine Umlaute." maxlength="255" required>
							</div>
